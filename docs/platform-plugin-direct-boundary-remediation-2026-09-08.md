...
- [ ] `CON-003` Audit Catalyst, Echo, Navigator and DH-System-Internal — **P1 / L**
  ...
  - Current result: audit is complete, but remediation remains open. Catalyst and Echo have no
    Platform service hop, yet both pin the retired Artifact SDK and closed Product kind projection.
    Navigator uses the retired `ArtifactKind.DATASET` in production and checks out Platform source
    during normal builds. DH is `NOT_APPLICABLE` to the Platform boundary.
...
| `CON-003` | Four live integration heads audited. Catalyst/Echo are direct at runtime but stale at the Artifact contract/build boundary; Navigator has a retired production API and Platform source checkout; DH is `NOT_APPLICABLE` to Platform coupling. Findings are routed only to `T20`, `T21`, `T22`, `T04` and `T23`. | Catalyst `#432`, Echo `#426` and Navigator `#433` succeeded only against their old pinned compositions; DH has no Azure pipeline | audit read-back complete; consumer remediation not started | `AUDIT_COMPLETE_REMEDIATION_OPEN` |
